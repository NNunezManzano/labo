...
modelo  <- rpart(formula=   "clase_binaria ~ . -clase_ternaria ",  #quiero predecir clase_ternaria a partir de el resto de las variables
                 ...
                 xval=      5,
                 cp=       -0.6082,   #esto significa no limitar la complejidad de los splits
                 minsplit=  718,     #minima cantidad de registros para que se haga el split
                 minbucket= 357,     #tamaño minimo de una hoja
                 maxdepth=  8 )    #profundidad maxima del arbol


#Con los datos de base aumentando maxdepth y min split obtengo mejores resultados.
