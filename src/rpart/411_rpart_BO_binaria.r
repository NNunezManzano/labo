...
#Completo los NA de Visa_mconsumototal
dataset[
  is.na(Visa_mconsumototal),
  Visa_mconsumototal:=0
]

#Completo los NA de Master_mconsumototal
dataset[
  is.na(Master_mconsumototal),
  Master_mconsumototal:=0
]

#Creo variable nn_deuda_total que acumula toda la deuda
...
#Agrego sobregiro de cuentas corrientes
dataset[
  mcuenta_corriente < 0,
  nn_deuda_total:= (
...
#Creo un Payroll total con las dos columnas de payroll
dataset[,
        nn_tpayroll := (
          mpayroll + mpayroll2
        )
...
#Utilizo todo lo anterior para determinar en cuantas veces el ingreso esta endeudado el cliente
dataset[
  nn_tpayroll > 0, 
...
dir.create("./exp/HT4112/", showWarnings = FALSE)
setwd("./exp/HT4112/") # Establezco el Working Directory DEL EXPERIMENTO

# defino los archivos donde guardo los resultados de la Bayesian Optimization
archivo_log <- "HT4112.txt"
archivo_BO <- "HT4112.RDATA"
