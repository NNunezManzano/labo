...
PARAM$experimento <- "HT7281"

PARAM$input$dataset <- "./datasets/rdd_competencia2_2022.csv.gz"
PARAM$input$training <- c(202103)

PARAM$trainingstrategy$undersampling <- 1.0 # un undersampling de 0.1  toma solo el 10% de los CONTINUA
PARAM$trainingstrategy$semilla_azar <- 103319 # Aqui poner la propia semilla 103319 / 104659 / 204019 / 307067 / 700067
...
PARAM$hyperparametertuning$semillas_azar <- c(103319, 104659, 204019, 307067, 700067) # Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy 103319 / 104659 / 204019 / 307067 / 700067
...
# modificar para que calcule 10 veces el 5fold
# vector_gan <- c()
# for (semilla in PARAM$SEMILLAS){
# vector_gan <- c(vector_gan, gan_normalizada)
# }
# return(MEAN(vactor_gan))
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  for (semilla in PARAM$hyperparametertuning$semillas_azar) {
    ...
    vector_gan <- c(vector_gan, ganancia_normalizada)
    vector_iter <- c(vector_iter, modelocv$best_iter)
  }

  ganancia_promedio <- mean(vector_gan)

  iter_promedio <- as.integer(mean(vector_iter))

  param_completo$num_iterations <- iter_promedio # asigno el mejor num_iterations
  param_completo["early_stopping_rounds"] <- NULL # elimino de la lista el componente  "early_stopping_rounds"

  # Voy registrando la importancia de variables
  if (ganancia_promedio > GLOBAL_gananciamax) {
    GLOBAL_gananciamax <<- ganancia_promedio
    modelo <- lgb.train(
      data = dtrain,
      param = param_completo,
      verbose = -100
  ...
  attr(ganancia_promedio, "extras") <- list("num_iterations" = iter_promedio) # esta es la forma de devolver un parametro extra

  # logueo
  xx <- param_completo
  xx$ganancia <- ganancia_promedio # le agrego la ganancia
